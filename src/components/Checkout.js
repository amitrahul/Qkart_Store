  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange={(e) => {
          handleNewAddress({ value: e.target.value, isAddingNewAddress: true });
        }}
      />
...
const Checkout = () => {
  ...
  const fetchCart = async (token) => {
    ...
    try {
      ...
      console.log("fetch cart item", response.data);
      return response.data;
    } catch {
      enqueueSnackbar(
  ...
  if(e.response){
    ...
    enqueueSnackbar("couldn't placed order, check backend is running",{variant : "error"})
  }
  }
  return false;
  };
    ...
    <>
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length > 0 ? (
                addresses.all.map((addressData) => (
                  <Box
                    key={addressData._id}
                    className={
                      addresses.selected === addressData._id
                        ? "address-item selected"
                        : "address-item not-selected"
                    }
                    onClick={() => {
                      setAddresses({ ...addresses, selected: addressData._id });
                    }}
                  >
                    ...
                    <Button
                      startIcon={<Delete />}
                      onClick={async () => {
                        await deleteAddress(token, addressData._id);
                      }}
                    >
                      Delete
                    </Button>
          ...
          {/* If newAddress.isAddingNewAddress is false, the code inside the curly braces will be rendered. Otherwise, nothing will be rendered.           */}
...
{/* If newAddress.isAddingNewAddress is true, the code inside the curly braces will be rendered. Otherwise, nothing will be rendered. */}
